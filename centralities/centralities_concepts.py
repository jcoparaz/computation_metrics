...
def pagerank_to_concepts(pmid_meshterms, pagerank_file, concept_pr_file_path):


    pagerank_cn = pd.read_csv(pagerank_file, names=["node", "pagerank_nk"], skiprows=1, delimiter='\t')
                                # ,
                                # dtype={"node": int, "ni": int, "nj":int, "nk":int, "disruption":float, "in":int, "out":int, "confidence":float})
    pagerank_cn = pagerank_cn.dropna()
    # print(disruption_cn.columns)

    cols = ['node', 'pagerank_nk']
    pagerank_values = pagerank_cn[cols].values
    num_articles = len(pagerank_cn)
    meshterms_disruption = {}
    for node, pagerank in pagerank_values:
        node = str(int(node))
        ...
        for mesh_descriptor in mesh_terms_txt:
            ...
            meshterms_disruption[mesh_descriptor] += pagerank
    meshterms_disruption = {mt: meshterms_disruption[mt] / num_articles for mt in meshterms_disruption}

    meshterms_disruption_pd = pd.DataFrame.from_dict(meshterms_disruption, orient='index', columns=['pagerank'])
    meshterms_disruption_pd.to_csv(concept_pr_file_path, index=True)
    print('{} created'.format(concept_pr_file_path))
...
def build_hierarchy(meshterms_pagerank, tree_numbers, root):
    G = nx.DiGraph()
    ...
    tree_node_dis = {}
    for mesh_term, disruption in meshterms_pagerank.items():
        meshui_treenum = {}
...
def compute_disruption_tree_nodes(G, tree_node_pr, tree_node_pr_file_path, pagerank_hierarchy_file_path):
    tree_node_pr_pd = pd.DataFrame.from_dict(tree_node_pr, orient='index', columns=['pagerank'])
    tree_node_pr_pd.to_csv(tree_node_pr_file_path, index=True)
    print('{} created'.format(tree_node_pr_file_path))
            ...
            for child in children:
                ...
                if child in tree_node_pr:
                    ...
                    if len(list(G[child])) == 0:
                        pagerank_hierarchy[child] = tree_node_pr[child]
                # disruption_hierarchy[child] to use the latest disruption value computed so far
                pr_parent += pagerank_hierarchy[child]
            pr_parent = pr_parent/len(depth_treenodes[current_level])
            if parent not in pagerank_hierarchy:
                # direct_disruptiveness is the disruption value if 'parent' node already has disruption
                direct_pagerank = tree_node_pr[parent] if parent in tree_node_pr else 0
                pagerank_hierarchy[parent] = pr_parent + direct_pagerank

    pagerank_hierarchy_pd = pd.DataFrame.from_dict(pagerank_hierarchy, orient='index', columns=['pagerank'])
    pagerank_hierarchy_pd.to_csv(pagerank_hierarchy_file_path, index=True)
    print('{} created'.format(pagerank_hierarchy_file_path))
    # print('!!!')
...
def propagate_centrality_cn_hierarchy_sample():
    # disruption_file = '/data/user/copara/dataset/PubMed/2022/metrics/disruption/1951/di_seq_1951_04.csv',
    # /home/cineca/Documents/dataset/PubMed/2022/metrics_sample/pagerank/2021/pagerank_2021_05.csv
    main_folder_centrality_pr = '/home/cineca/Documents/dataset/PubMed/2022/metrics_sample/pagerank'
    ...
    for year in range(year_s, year_e + 1):
        for month in range(month_s, month_e + 1):
            ...
            month = str(month)
            month = '0' + month if len(month) == 1 else month
            folder_centrality_year = '{}/{}'.format(main_folder_centrality_pr, year)
            # disruption_file = '/data/user/copara/dataset/PubMed/2022/metrics/disruption/{}/di_seq_{}_{}{}.csv'.format(year, year, month, sample_lbl)
            centrality_file = '{}/pagerank_{}_{}.csv'.format(folder_centrality_year, year, month)
            concept_centr_file_path = '{}/concept_pagerank_{}_{}.csv'.format(folder_centrality_year, year, month)
            treenode_centr_initial_file_path = '{}/treenode_pagerank_initial_{}_{}.csv'.format(folder_centrality_year, year, month)
            treenode_centr_file_path = '{}/treenode_pagerank_{}_{}.csv'.format(folder_centrality_year, year, month)
            disruption_concepts = pagerank_to_concepts(pmid_meshterms=pmid_meshterms, pagerank_file=centrality_file, concept_pr_file_path=concept_centr_file_path)
            tree_numbers = load_tree_numbers(tree_numbers_file_path=treenumbers_path)
            graph_hierarchy, tree_node_disruption = build_hierarchy(meshterms_pagerank=disruption_concepts,
                                                                    tree_numbers=tree_numbers, root=root)
            compute_disruption_tree_nodes(G=graph_hierarchy, tree_node_pr=tree_node_disruption,
                                          tree_node_pr_file_path=treenode_centr_initial_file_path,
                                          pagerank_hierarchy_file_path=treenode_centr_file_path)
            # aggregated_tree_nodes = aggregate_bottom_top(tree_node_value=tree_node_disruption, root=root,
            #                                              num_concepts=len(disruption_concepts),
            #                                              treenode_disruption_file_path=treenode_di_file_path)
            # print('!!!')
...
def propagate_centrality_cn_hierarchy_sample_process(source_folder_centrality, destination_folder_centrality,
    ...
    source_folder_centrality_year = '{}/{}'.format(source_folder_centrality, year)
    destination_folder_centrality_year = '{}/{}'.format(destination_folder_centrality, year)
    if not os.path.exists(destination_folder_centrality_year):
        os.makedirs(destination_folder_centrality_year)
    centrality_file = '{}/pagerank_{}_{}.csv'.format(source_folder_centrality_year, year, month)
    concept_centr_file_path = '{}/concept_pagerank_{}_{}.csv'.format(destination_folder_centrality_year, year, month)
    treenode_centr_initial_file_path = '{}/treenode_pagerank_initial_{}_{}.csv'.format(destination_folder_centrality_year, year, month)
    treenode_centr_file_path = '{}/treenode_pagerank_{}_{}.csv'.format(destination_folder_centrality_year, year, month)
    centrality_concepts = pagerank_to_concepts(pmid_meshterms=pmid_meshterms_year, pagerank_file=centrality_file,
                                                 concept_pr_file_path=concept_centr_file_path)
    # tree_numbers = load_tree_numbers(tree_numbers_file_path=tree_numbers_year)
    graph_hierarchy, tree_node_centrality = build_hierarchy(meshterms_pagerank=centrality_concepts,
                                                            tree_numbers=tree_numbers_year, root=root)
    compute_disruption_tree_nodes(G=graph_hierarchy, tree_node_pr=tree_node_centrality,
                                  tree_node_pr_file_path=treenode_centr_initial_file_path,
                                  pagerank_hierarchy_file_path=treenode_centr_file_path)
    # aggregated_tree_nodes = aggregate_bottom_top(tree_node_value=tree_node_disruption, root=root,
    #                                              num_concepts=len(disruption_concepts),
...
def propagate_centrality_cn_hierarchy_sample_pmyears():
    pubmed_folder = '../data'
    source_folder_centrality = '../output/metrics_sample/pagerank'
    destination_folder_centrality = '../output/metrics_sample/pagerank'
    ...
    for year in range(year_s, year_e + 1):
        ...
        for month in range(month_s, month_e + 1):
            ...
            propagate_centrality_cn_hierarchy_sample_process(source_folder_centrality, destination_folder_centrality,
                                                             year, month, pmid_meshterms_year, tree_numbers_year)


if __name__ == '__main__':
    # propagate_centrality_cn_hierarchy_sample()
    propagate_centrality_cn_hierarchy_sample_pmyears()